...
At first sight, I noticed that this challenge sounds like a half decrypted php sourcecode
I searched for php opcodes and I found this: http://www.php.net/manual/en/internals2.opcodes.list.php

by installing php5vld and fuzzing it I got the sourcecode below:

```
<?php
  error_reporting(0);
  require_once("flag.php");
  $addr = $_GET[md5($_SERVER['REMOTE_ADDR'])];
  $user = $_GET[md5($_SERVER['HTTP_USER_AGENT'])];
  if($addr != $user){
    if(md5($addr) === hash("md5",$user)){
      if(hash("sha512",$addr,true)!=hash("sha512",$user,true)){
        echo "<br><br>You got it here is your flag: " . FLAG;
      }
    }
  }
?>
...
I found this as md5 collision and by googling you can get the string (http://www.links.org/?p=6)
at the end, you get the flag by inserting the manipulated variables.
...
```
[root@localhost test]# php -d vld.active=1 -d vld.execute=0 -f index.php
Finding entry points
Branch analysis from position: 0
Jump found. Position 1 = 19, Position 2 = 42
Branch analysis from position: 19
...
Branch analysis from position: 41
Branch analysis from position: 42
filename:       /root/test/index.php
function name:  (null)
number of ops:  43
compiled vars:  !0 = $addr, !1 = $user
line     # *  op                           fetch          ext  return  operands
---------------------------------------------------------------------------------
   2     0  >   SEND_VAL                                                 0
         1      DO_FCALL                                      1          'error_reporting'
   ...
   8    26  >   SEND_VAL                                                 'sha512'
        ...
        34      IS_NOT_EQUAL                                     ~20     $18, $19
        35    > JMPZ                                                     ~20, ->40
   9    36  >   FETCH_CONSTANT                                   ~21     'FLAG'
        37      CONCAT                                           ~22     '%3Cbr%3E%3Cbr%3EYou+got+it+here+is+your+flag%3A+', ~21
        38      ECHO                                                     ~22
  10    39    > JMP                                                      ->40
  11    40  > > JMP                                                      ->41
  12    41  > > JMP                                                      ->42
...
branch: #  0; line:     2-    6; sop:     0; eop:    18; out1:  19; out2:  42
branch: # 19; line:     7-    7; sop:    19; eop:    25; out1:  26; out2:  41
branch: # 26; line:     8-    8; sop:    26; eop:    35; out1:  36; out2:  40
branch: # 36; line:     9-   10; sop:    36; eop:    39; out1:  40
branch: # 40; line:    11-   11; sop:    40; eop:    40; out1:  41
branch: # 41; line:    12-   12; sop:    41; eop:    41; out1:  42
branch: # 42; line:    14-   14; sop:    42; eop:    42
path #1: 0, 19, 26, 36, 40, 41, 42,
path #2: 0, 19, 2, 40, 41, 42,
path #3: 0, 19, 41, 42,
path #4: 0, 42,
[root@localhost test]#
```